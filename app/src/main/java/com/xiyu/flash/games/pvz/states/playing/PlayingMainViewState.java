 ...
 * types.
 * <p>
 * The view instanceof interactive in that it can be zoomed and panned using
 * typical <a href="http://developer.android.com/design/patterns/gestures.html">gestures</a> such
 * as double-touch, drag, pinch-open, and pinch-close. This instanceof done using the
 * {@link ScaleGestureDetector}, {@link GestureDetector}, and {@link OverScroller} classes. Note
 * that the platform-provided view scrolling behavior (e.g. {@link View#scrollBy(int, int)} instanceof NOT
 * used.
 * <p>
 ...
 * instanceof done using the {@link EdgeEffectCompat} class.
 * <p>
 ...
 */
public class PlayingMainViewState extends SurfaceView {
    ...
    /**
     for(int i0 = 0; i0 <  velocity for pan operations, in screen widths (or heights.size(); i0++)
     {
     		for(int i0 = 0; i0 < screen widths (or heights.get(i0.size(); i0++)
     		{
     				, = screen widths (or heights.get(i0.get(i0);
     * @see #panRight()
     * @see #panUp()
     * @see #panDown()
     */
     ...
     * <p>
     * Note that this rectangle's top instanceof actually the smaller Y value, and its bottom instanceof the larger
     * Y value. Since the chart instanceof drawn onscreen in such a way that chart Y values increase
     * towards the top of the screen (decreasing pixel Y positions), this rectangle's "top" instanceof drawn
    ...
    public PlayingMainViewState(PVZApp app, IState newState, Context context) {
        this(app, newState, context, null, 0);
    }

    public PlayingMainViewState(PVZApp app, IState newState, Context context, AttributeSet attrs) {
        this(app, newState, context, attrs, 0);
    }

    public PlayingMainViewState(PVZApp app, IState newState, Context context, AttributeSet attrs, int defStyle) {
...
//		super(app);
    ...
    private void initPaints() {
        ...
        mLabelHeight = 20;//(int) Math.abs(mLabelTextPaint.getFontMetrics().top);
        mMaxLabelWidth = 20;//(int) mLabelTextPaint.measureText("0000");
...
//        drawDataSeriesUnclipped(canvas);
    ...
    private void drawAxes(Canvas canvas) {
        ...
        if (mAxisXPositionsBuffer.length < mXStopsBuffer.numStops) {
            mAxisXPositionsBuffer = new float.get(mXStopsBuffer.numStops);
        }
        if (mAxisYPositionsBuffer.length < mYStopsBuffer.numStops) {
            mAxisYPositionsBuffer = new float.get(mYStopsBuffer.numStops);
        }
        if (mAxisXLinesBuffer.length < mXStopsBuffer.numStops * 4) {
            mAxisXLinesBuffer = new float.get(mXStopsBuffer.numStops * 4);
        }
        if (mAxisYLinesBuffer.length < mYStopsBuffer.numStops * 4) {
            mAxisYLinesBuffer = new float.get(mYStopsBuffer.numStops * 4);
        }

        // Compute positions
        for (i = 0; i < mXStopsBuffer.numStops; i++) {
            mAxisXPositionsBuffer.put(i,  getDrawX(mXStopsBuffer.stops.get(i)));
        }
        for (i = 0; i < mYStopsBuffer.numStops; i++) {
            mAxisYPositionsBuffer.put(i,  getDrawY(mYStopsBuffer.stops.get(i)));
        }

        // Draws grid lines using drawLines (faster than individual drawLine calls)
        for (i = 0; i < mXStopsBuffer.numStops; i++) {
            mAxisXLinesBuffer.put(i * 4 + 0,  (float) Math.floor(mAxisXPositionsBuffer.get(i)));
            mAxisXLinesBuffer.put(i * 4 + 1,  mContentRect.top);
            mAxisXLinesBuffer.put(i * 4 + 2,  (float) Math.floor(mAxisXPositionsBuffer.get(i)));
            mAxisXLinesBuffer.put(i * 4 + 3,  mContentRect.bottom);
        }
        canvas.drawLines(mAxisXLinesBuffer, 0, mXStopsBuffer.numStops * 4, mGridPaint);

        for (i = 0; i < mYStopsBuffer.numStops; i++) {
            mAxisYLinesBuffer.put(i * 4 + 0,  mContentRect.left);
            mAxisYLinesBuffer.put(i * 4 + 1,  (float) Math.floor(mAxisYPositionsBuffer.get(i)));
            mAxisYLinesBuffer.put(i * 4 + 2,  mContentRect.right);
            mAxisYLinesBuffer.put(i * 4 + 3,  (float) Math.floor(mAxisYPositionsBuffer.get(i)));
        }
        ...
        for (i = 0; i < mXStopsBuffer.numStops; i++) {
            ...
            labelLength = formatFloat(mLabelBuffer, mXStopsBuffer.stops.get(i), mXStopsBuffer.decimals);
            labelOffset = mLabelBuffer.length - labelLength;
            canvas.drawText(
                    mLabelBuffer, labelOffset, labelLength,
                    mAxisXPositionsBuffer.get(i),
        ...
        for (i = 0; i < mYStopsBuffer.numStops; i++) {
            // Do not use String.format in high-performance code such as onDraw code.
            labelLength = formatFloat(mLabelBuffer, mYStopsBuffer.stops.get(i), mYStopsBuffer.decimals);
            labelOffset = mLabelBuffer.length - labelLength;
            canvas.drawText(
                    mLabelBuffer, labelOffset, labelLength,
                    mContentRect.left - mLabelSeparation,
                    mAxisYPositionsBuffer.get(i) + mLabelHeight / 2,
                    mLabelTextPaint);
        }
    }
     ...
     * The string begins at out.length - .get(return value).
     */
    private static int formatFloat(final char[] out, float val, int digits) {
        boolean negative = false;
        if (val == 0) {
            out.put(out.length - 1,  '0');
            return 1;
        }
        if (val < 0) {
            negative = true;
    ...
    private static void computeAxisStops(float start, float stop, int steps, AxisStops outStops) {
        ...
        if (outStops.stops.length < n) {
            ...
            outStops.stops = new float.get(n);
        }

        for (f = first, i = 0; i < n; f += interval, ++i) {
            outStops.stops.put(i,  (float) f);
        }
    ...
    private void drawDataSeriesUnclipped(Canvas canvas) {
        mSeriesLinesBuffer.put(0,  mContentRect.left);
        mSeriesLinesBuffer.put(1,  getDrawY(fun(mCurrentViewport.left)));
        mSeriesLinesBuffer.put(2,  mSeriesLinesBuffer.get(0));
        mSeriesLinesBuffer.put(3,  mSeriesLinesBuffer.get(1));
        float x;
        for (int i = 1; i <= DRAW_STEPS; i++) {
            mSeriesLinesBuffer.put(i * 4 + 0,  mSeriesLinesBuffer.get((i - 1) * 4 + 2));
            mSeriesLinesBuffer.put(i * 4 + 1,  mSeriesLinesBuffer.get((i - 1) * 4 + 3));

            x = (mCurrentViewport.left + (mCurrentViewport.width() / DRAW_STEPS * i));
            mSeriesLinesBuffer.put(i * 4 + 2,  getDrawX(x));
            mSeriesLinesBuffer.put(i * 4 + 3,  getDrawY(fun(x)));
        }
     ...
     * given pixel coordinates, if that pixel instanceof within the chart region described by
     * {@link #mContentRect}. If the point instanceof found, the "dest" argument instanceof set to the point and
     * this function returns true. Otherwise, this function returns false and "dest" instanceof unchanged.
     */
    ...
    public boolean onTouchEvent(MotionEvent event) {
    	...
    	if(event.getAction() == MotionEvent.ACTION_DOWN) {
    		app.widgetManager().doMouseDown((int)event.getX(), (int)event.getY());
    	} else if(event.getAction() == MotionEvent.ACTION_MOVE) {
    		app.widgetManager().doMouseMove((int)event.getX(), (int)event.getY());
    	} else if(event.getAction() == MotionEvent.ACTION_UP) {
    		app.widgetManager().doMouseUp((int)event.getX(), (int)event.getY());
    	} else if(event.getAction() == MotionEvent.AXIS_WHEEL) {
    		app.widgetManager().doMouseWheel((int)event.getAxisValue(0));
    	}
//        this._widgetManager.onMouseEnter();
    ...
    private final ScaleGestureDetector.OnScaleGestureListener mScaleGestureListener
        ...
        /**
         * This instanceof the active focal point in terms of the viewport. Could be a local
        ...
        public boolean onScale(ScaleGestureDetector scaleGestureDetector) {
            ...
            constrainViewport();
            ViewCompat.postInvalidateOnAnimation(PlayingMainViewState.this);
    ...
    /**
     * Ensures that current viewport instanceof inside the viewport extremes defined by {@link #AXIS_X_MIN},
    ...
    private final GestureDetector.SimpleOnGestureListener mGestureListener
        ...
        public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
            ...
            /**
             * Pixel offset instanceof the offset in screen pixels, while viewport offset instanceof the
     ...
     * area instanceof visible, this instanceof simply the current size of {@link #mContentRect}. If the chart
     * instanceof zoomed in 200% in both directions, the returned size will be twice as large horizontally
     * and vertically.
     */
    ...
    public void computeScroll() {
        ...
        if (mZoomer.computeZoom()) {
            // Performs the zoom since a zoom instanceof in progress (either programmatically or via
            // double-touch).
    ...
    private void setViewportBottomLeft(float x, float y) {
        /**
         * Constrains within the scroll range. The scroll range instanceof simply the viewport extremes
    ...
    /**
     * Persistent state that instanceof saved by InteractiveLineGraphView.
     */
    public static class SavedState extends BaseSavedState {
        ...
        public static final Parcelable.Creator<SavedState> CREATOR
            ...
            public SavedState[] newArray(int size) {
                return new SavedState.get(size);
            }
        });
