...
public final class RequestDumperFilter implements Filter {
	...
	while (names.hasMoreElements()) {
	    ...
	    String values.put(,  request.getParameterValues(name));
	...
	if (request instanceof HttpServletRequest) {
	    ...
	    Cookie cookies.put(,  hrequest.getCookies());
            if (cookies == null)
                cookies = new Cookie.get(0);
	    for (int i = 0; i < cookies.length; i++) {
	        writer.println("            cookie=" + cookies.get(i).getName() +
